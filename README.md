...
1. Click the button and connect the repo. On the Blueprint form, set **Blueprint Name** to `Launchkit Template` (or any name). The **Blueprint Path** field is often not prefilled—type `render.yaml` so Render finds the file at the repo root. Then create the Blueprint. Render will create `launchkit-db`, `launchkit-api`, and `launchkit-web`.
   ...
   - **If you see "Blueprint file render.yaml not found on main branch":** This repo has `render.yaml` at the repo root on `main`. Confirm you're connecting **this** repo (`aculich/launch-kit-spa-desktop-switchdimension`) and that **Branch** is `main`; keep **Blueprint Path** as `render.yaml` (not a subdirectory). Ensure Render has access to the repo (in GitHub: Settings → Applications → Render, or re-authorize when Render prompts). Click **Retry** on the Blueprint page. If you deploy from a different fork, push `render.yaml` to that fork's `main` first. A `.render-ignore` at repo root can also hide the file—remove it if present.
